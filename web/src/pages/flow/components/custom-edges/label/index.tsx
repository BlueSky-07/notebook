...
import { BaseEdge, EdgeProps, Edge, EdgeLabelRenderer, getStraightPath, Position } from '@xyflow/react';
...
export const LabelEdge = (props: CustomEdgeLabelProps) => {
  const { data, id, targetY, targetX, sourceY, sourceX, markerStart, markerEnd, sourcePosition, targetPosition } = props
  ...
  const edgePath = [
    `M ${sourceX} ${sourceY}`,
    sourcePosition === Position.Right && `L ${sourceX + (sourcePosition === Position.Right ? fixHorizontalArrow : 0)} ${sourceY}`,
    ...
    targetPosition === Position.Left && `L ${targetX - (targetPosition === Position.Left ? fixHorizontalArrow : 0)} ${targetY}`,
    `L ${targetX} ${targetY}`
  ].filter(Boolean).join(' ')
